...
class ProceduralEffectRuntime(private val effect: PlayboxEffect) {
    ...
    private val bloom = (spec as? ProceduralSpec.OrganicBloom)?.let(::BloomSimulation)
    private val stars = (spec as? ProceduralSpec.Starfield)?.let(::StarfieldSimulation)
    ...
    fun frameAt(elapsedMs: Long): EffectFrame = when (val current = spec) {
        ...
        is ProceduralSpec.OrganicBloom -> requireNotNull(bloom).frameAt(elapsedMs)
        is ProceduralSpec.RippleField -> rippleFrame(current, elapsedMs.coerceAtLeast(0))
        is ProceduralSpec.Starfield -> requireNotNull(stars).frame(elapsedMs.coerceAtLeast(0))
    }
